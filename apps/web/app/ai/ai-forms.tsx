...
export function AiExtractionCreateForm() {
    ...
    try {
      ...
      if (mode === "upload") {
        ...
        if (!["txt", "md"].includes(extension ?? "")) {
          throw new Error(
            "PDF, Word, and scanned contract parsing need the document parser service. Paste text or upload .txt/.md for now.",
          );
        }
      ...
      if (text.trim().length < 20) {
        throw new Error(
          "Add at least 20 characters of contract text before extraction",
        );
      }

      const toastId = toast.loading("Extraction started", {
        description: "RevFlow is reading the contract terms.",
      });
      ...
      toast.success("Extraction ready for review", {
        id: toastId,
        description: "Open the run below to resolve fields and evidence.",
      });
      setIsOpen(false);
      setSourceName("");
      setSourceText("");
  ...
  return (
    <>
      <button
        className="primary-link ai-new-extraction-button"
      ...
      {isOpen ? (
        <div className="ai-modal-backdrop" role="presentation">
          <section
            aria-labelledby="ai-new-extraction-title"
            aria-modal="true"
            className="ai-extraction-modal"
            role="dialog"
          >
            <div className="panel-title-row">
              <div>
                ...
                <p>
                  Start from pasted contract text today. Document upload is
                  staged for the parser service.
                </p>
              </div>
              <button
            ...
            <form className="ai-modal-form" onSubmit={onSubmit}>
              ...
              {mode === "paste" ? (
                ...
                <div className="ai-upload-dropzone">
                  ...
                  <strong>
                    {selectedFile
                      ? selectedFile.name
                      : "Drop in a contract file"}
                  </strong>
                  <span>
                    .txt and .md can be read now. PDF, Word, and scanned files
                    are queued for the document parser service.
                  </span>
                </div>
              )}
              ...
              <div className="ai-modal-actions">
                <span>
                  {mode === "paste"
                    ? "Paste terms, then create a review run."
                    : "Upload path is staged for richer document parsing."}
                </span>
...
function parseReviewedValue(
  original: AiExtractedValue,
  draft: FieldDraft,
): AiExtractedValue {
...
export function AiReviewWorkbench({
  ...
  extractions: AiExtractionRun[];
}) {
  const router = useRouter();
  const initialSelection =
    extractions.find((run) => run.status === "extracted")?.id ??
    extractions[0]?.id ??
    "";
  ...
  const baseOutput =
    selected?.reviewedOutput ?? selected?.structuredOutput ?? null;
  ...
  const pendingCount = fields.filter(
    (field) => (drafts[field.key]?.status ?? "pending") === "pending",
  ).length;
  ...
  async function submitReview(status: "approved" | "rejected") {
    ...
    try {
      ...
      const response = await fetch(
        `/api/ai/extractions/${selected.id}/review`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify(payload),
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body?.message ??
            `Could not ${status === "approved" ? "approve" : "reject"} extraction`,
        );
      }
  ...
  async function applyExtraction() {
    ...
    if (
      !window.confirm(
        "Apply the approved extraction into draft customer and contract records? The resulting contract still requires normal activation controls.",
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/ai/extractions/${selected.id}/apply`, {
        method: "POST",
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(
          body?.message ?? "Could not apply extraction to draft configuration",
        );
      }
    ...
    <section className="ai-review-panel">
      ...
      <div className="ai-review-grid">
        ...
        <div className="extraction-panel">
          ...
          {fields.length === 0 ? (
            <p className="empty-state">No extracted fields available.</p>
          ) : null}
          <div className="extracted-fields">
            {fields.map((field) => {
              ...
              return (
                <div
                  className={`extracted-field ${lowConfidence ? "low-confidence" : ""}`}
                  key={field.key}
                >
                  <div className="field-heading">
                    ...
                    <div className="field-status-group">
                      ...
                      <span
                        className={`decision-status decision-${draft.status}`}
                      >
                        {draft.status}
                      </span>
                    </div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setEditedValue(field, event.target.value)
                    }
                    value={draft.value}
                  />
                  ...
                  {field.sourceSnippet ? (
                    <p className="source-snippet">
                      <strong>Source evidence:</strong> {field.sourceSnippet}
                    </p>
                  ) : null}
                  {field.ambiguity ? (
                    <p className="ambiguity-text">{field.ambiguity}</p>
                  ) : null}

                  {canReview ? (
                    <div
                      className="field-actions"
                      aria-label={`Review actions for ${field.label}`}
                    >
                      <button
                      ...
                      <button
                        ...
                        onClick={() => setDecision(field, "edited")}
                        title="Edit value"
                        type="button"
                      >
                        Edit
            ...
            <div className="review-footer">
              ...
              <div className="review-actions">
                <span>
                  {pendingCount === 0
                    ? "All fields resolved"
                    : `${pendingCount} fields still pending`}
                </span>
                <button
          ...
          {canApply ? (
            <div className="apply-banner">
              <div>
                ...
                <span>
                  Customer and contract records remain subject to normal
                  activation controls.
                </span>
              </div>
              <button
          ...
          {selected?.status === "applied" && selected.appliedContractId ? (
            <div className="apply-banner applied-banner">
              <div>
                ...
                <a href={`/contracts/${selected.appliedContractId}`}>
                  {selected.appliedContractId}
                </a>
              </div>
            </div>
          ) : null}
